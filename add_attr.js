
import { Client, Databases } from 'node-appwrite';

const endpoint = "https://fra.cloud.appwrite.io/v1";
const projectId = "studentcompanion";
const apiKey = "standard_";

const client = new Client()
    .setEndpoint(endpoint)
    .setProject(projectId)
    .setKey(apiKey);

const databases = new Databases(client);

const DATABASE_ID = "studentcompanion";
const MESSAGES_COL_ID = "69945f67003350f65039";

async function addAttribute() {
    console.log("Adding attachmentNames attribute...");
    try {
        await databases.createStringAttribute(DATABASE_ID, MESSAGES_COL_ID, "attachmentNames", 255, false, undefined, true);
        console.log("Attribute created!");
    } catch (e) {
        console.log("Attribute might already exist or error:", e.message);
    }
}

addAttribute();
