...
const Communication: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'chats' | 'help' | 'tasks' | 'contacts'>('chats');
    ...
    const fetchAllContacts = async () => {
        if (allContacts.length > 0) return; // Only fetch once or when forced
        ...
        try {
            const response = await databases.listDocuments(
                APPWRITE_CONFIG.DATABASE_ID,
                APPWRITE_CONFIG.PROFILES_COLLECTION_ID,
                [Query.limit(100), Query.orderAsc('nickname')]
            );
            setAllContacts(response.documents);
    ...
    useEffect(() => {
        ...
        const filtered = allContacts.filter(contact =>
            contact.nickname?.toLowerCase().includes(contactSearchQuery.toLowerCase()) ||
            contact.email?.toLowerCase().includes(contactSearchQuery.toLowerCase()) ||
            contact.matricNumber?.toLowerCase().includes(contactSearchQuery.toLowerCase())
        );
        setFilteredContacts(filtered);
    }, [contactSearchQuery, allContacts]);
    ...
    return (
        <div className="flex flex-col h-[100dvh] bg-slate-50 relative pb-safe">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between px-4 py-3 md:px-6 md:py-4 bg-white/80 backdrop-blur-md border-b border-slate-200/50 sticky top-0 z-10 shrink-0">
                ...
                <div className="flex space-x-1 bg-slate-100 p-1 rounded-lg">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-medium rounded-md transition-all ${activeTab === 'contacts' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Contacts
                    </button>
                </div>
            ...
            <div className="flex-1 overflow-hidden p-0 md:p-6">
                ...
                {activeTab === 'contacts' && (
                    <div className="flex flex-col h-full bg-white md:rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="p-4 md:p-6 border-b border-slate-100 bg-slate-50/50">
                            <h2 className="text-xl font-bold text-slate-900 mb-1">Classmates Directory</h2>
                            <p className="text-xs text-slate-500 mb-4">Find and connect with other students</p>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-white">
                            {isContactsLoading ? (
                                <div className="h-64 flex flex-col items-center justify-center gap-3">
                                    ...
                                    <p className="text-sm text-slate-400 font-medium">Loading directory...</p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {filteredContacts.map(contact => (
                                        <div
                                            key={contact.$id}
                                            className="group p-4 bg-white rounded-2xl border border-slate-100 hover:border-indigo-100 hover:shadow-md transition-all duration-200"
                                        >
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
                                                    {contact.nickname?.[0]?.toUpperCase() || "U"}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h3 className="font-bold text-slate-800 truncate">{contact.nickname || "Anonymous User"}</h3>
                                                    <p className="text-[10px] text-slate-400 truncate">{contact.email || "No email"}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex gap-2">
                                                <button
                                                    ...
                                                    className="flex-1 py-2 bg-slate-50 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2"
                                                >
                                                    <Send className="w-3.5 h-3.5" />
                                                    Message
                                                </button>
                                                <button
                                                    onClick={() => initiateCall(contact.userId)}
                                                    className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 shadow-sm hover:shadow-indigo-100"
                                                >
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-center p-8 py-20">
                                    ...
                                    <h3 className="text-lg font-bold text-slate-900 mb-1">No classmates found</h3>
                                    <p className="text-slate-500 text-sm max-w-xs mx-auto">
                                        We couldn't find any users matching your search. Try a different name or matric number.
                                    </p>
                                </div>
                            )}
                        </div>
